...
var page = {
	...
	renderModelView: function(showDeleteButton)	{
		...
		// populate the dropdown options for clienteCId
		// TODO: load only the selected value, then fetch all options when the drop-down is clicked
		var clienteCIdValues = new model.ClienteCollection();
		clienteCIdValues.fetch({
			success: function(c){
				var dd = $('#clienteCId');
				...
				c.forEach(function(item,index) {
					dd.append(app.getOptionHtml(
						...
						page.agenda.get('clienteCId') == item.get('id')
					));
				});
		...
		// populate the dropdown options for justicaJId
		// TODO: load only the selected value, then fetch all options when the drop-down is clicked
		var justicaJIdValues = new model.JusticaCollection();
		justicaJIdValues.fetch({
			success: function(c){
				var dd = $('#justicaJId');
				dd.append('<option value=""></option>');
				c.forEach(function(item,index) {
					dd.append(app.getOptionHtml(
						item.get('id'),
						item.get('nome'), // TODO: change fieldname if the dropdown doesn't show the desired column
						page.agenda.get('justicaJId') == item.get('id')
					));
				});
		...
		// populate the dropdown options for funcionarioFId
		// TODO: load only the selected value, then fetch all options when the drop-down is clicked
		var funcionarioFIdValues = new model.FuncionarioCollection();
		funcionarioFIdValues.fetch({
			success: function(c){
				var dd = $('#funcionarioFId');
				dd.append('<option value=""></option>');
				c.forEach(function(item,index) {
					dd.append(app.getOptionHtml(
						item.get('id'),
						item.get('nome'), // TODO: change fieldname if the dropdown doesn't show the desired column
						page.agenda.get('funcionarioFId') == item.get('id')
					));
				});
	...
	updateModel: function() {
		...
		page.agenda.save({
			...
			'aProcesso': $('input#aProcesso').val(),
			'clienteCId': $('select#clienteCId').val(),
			'justicaJId': $('select#justicaJId').val(),
			'funcionarioFId': $('select#funcionarioFId').val(),
			'aDataIn': $('input#aDataIn').val(),
			'aDateOut': $('input#aDateOut').val(),
			'aObservacao': $('textarea#aObservacao').val(),
			'estatus': $('select#estatus').val()
		}, {
			wait: true,
			success: function(){
