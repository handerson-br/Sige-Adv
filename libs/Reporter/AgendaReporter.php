	...
	*/
	static function GetCustomQuery($criteria)
	{
		$sql = "select
			...
			,`agenda`.`a_id` as AId
			,`agenda`.`a_processo` as AProcesso
			,`agenda`.`cliente_c_id` as ClienteCId
			,`agenda`.`justica_j_id` as JusticaJId
			,`agenda`.`funcionario_f_id` as FuncionarioFId
			,`agenda`.`a_data_in` as ADataIn
			,`agenda`.`a_date_out` as ADateOut
			,`agenda`.`a_observacao` as AObservacao
			,`agenda`.`estatus` as Estatus
		from `agenda`";
